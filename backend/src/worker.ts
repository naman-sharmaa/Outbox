...
const worker = new Worker(
  ...
  async (job: Job) => {
    ...
    if (updatedJob.count === 0) {
      console.log(
        `Job ${rowId} is not in scheduled state. Skipping (idempotency hit).`,
      );
      return;
    }
    ...
    if (currentCount > maxEmails) {
      ...
      const newJobId = crypto
        .createHash("sha256")
        .update(dbJob.id + nextHour.toISOString())
        .digest("hex");
    ...
    try {
      ...
      console.log(
        `Email sent: ${info.messageId} - Preview: ${nodemailer.getTestMessageUrl(info)}`,
      );
